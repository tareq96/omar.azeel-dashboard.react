...
import { Check, Info, MapPin, ClipboardList, PlusIcon, LoaderCircleIcon } from "lucide-react";
import {
  useLockersStore,
...
export default function AddLockerDialog({ onCreated }: AddLockerDialogProps) {
  ...
  const lockersStoreMutation = useLockersStore();
  ...
  const customersOptions = useMemo<CustomerOption[]>(() => {
    ...
    if (!apiCustomers || !apiZones) return [];
  ...
  React.useEffect(() => {
    if (!open) {
      ...
      setCustomersSearch("");
      setActiveStep(1);
    } else if (formDataQuery.data?.data) {
      // Pre-fill code from API
      if (formDataQuery.data.data.locker_code) {
        setCode(formDataQuery.data.data.locker_code);
      }
      // Pre-select first status if available
      if (formDataQuery.data.data.statuses && formDataQuery.data.data.statuses.length > 0) {
        setStatus(formDataQuery.data.data.statuses[0].key);
      }
    }
  }, [open, formDataQuery.data]);
  ...
  const isSubmitting = lockersStoreMutation.isPending;
  ...
  const handleCreateLocker = async () => {
    if (!canProceedInfo || !canProceedAddress) return;
    try {
      await lockersStoreMutation.mutateAsync({
        data: {
          code,
          status,
          user_id: customerId!,
      ...
      toast.success(
        t("lockers.addDialog.success", { defaultValue: "Locker has been created successfully" }),
      );
      try {
      ...
      const translated =
        rawMessage || t("lockers.addDialog.error", { defaultValue: "Failed to create locker" });
    ...
    <ResponsiveDialog open={open} onOpenChange={(o) => setOpen(o)}>
      <ResponsiveDialog.Trigger asChild>
        <Button>
          ...
          {t("lockers.addDialog.trigger", { defaultValue: "Add Locker" })}
        </Button>
      </ResponsiveDialog.Trigger>
      <ResponsiveDialog.Content className="max-w-2xl">
        <ResponsiveDialog.Header>
          <ResponsiveDialog.Title>
            {t("lockers.addDialog.title", { defaultValue: "Add Locker" })}
          </ResponsiveDialog.Title>
        </ResponsiveDialog.Header>
        ...
        <ResponsiveDialog.Footer>
          <StepperFooterControls
            ...
            t={t}
            labels={{
              submit: t("lockers.addDialog.confirmation.create", { defaultValue: "Create Locker" }),
            }}
          />
        </ResponsiveDialog.Footer>
