import { useCreditsCreditsCardsList } from "@/services/api/generated/credits/credits";
import type { CreditRow } from "@/components/credits-list/types";

type CreditsCardsApiResponse = {
  current_page?: number;
  data?: CreditRow[];
  total?: number;
  per_page?: number;
...
export function useCreditsCardsData({
  search,
  from_date,
  to_date,
}: {
  ...
  const { data, isPending } = useCreditsCreditsCardsList<CreditsCardsApiResponse>(
    {
